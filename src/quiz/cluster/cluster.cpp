...
void Proximity(std::vector<float> point,std::vector<int>& cluster,int point_id,std::vector<int> &status,KdTree* tree,float distanceTol)
{
	status[point_id]=0;
	std::vector<int> near_point = tree->search(point, distanceTol);
...
std::vector<std::vector<int>> euclideanCluster(const std::vector<std::vector<float>>& points, KdTree* tree, float distanceTol)
{
	...
	std::vector<int> status(points.size(),0);
	for(int i=0; i<points.size();i++)
	{
		...
		if(status[i]==0)
		{
			...
			Proximity(point,cluster,point_id, status,tree,distanceTol);
...
int main ()
{
    ...
    // tree->insert(points[0],1);
	//tree->insert(points[1],2);  
  	int it = 0;
